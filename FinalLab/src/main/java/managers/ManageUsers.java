...
public class ManageUsers {
	...
	public User getUser(Integer id) {
		String query = "SELECT id,name,usr,mail FROM users WHERE id = ? ;";
	...
	public void addUser(User user) {
		String query = "INSERT INTO users (name,usr,mail,pwd,doB,vRequest,gender,usrType) VALUES (?,?,?,?,?,?,?,?)";
		PreparedStatement statement = null;
		try {
			LocalDate ld = LocalDate.parse(user.getDbirth());
			statement = db.prepareStatement(query);
			statement.setString(1,user.getName());
			statement.setString(2,user.getUser());
			statement.setString(3,user.getMail());
            statement.setString(4,user.getPwd1());
            statement.setDate(5,java.sql.Date.valueOf(ld),Calendar.getInstance());
            statement.setInt(6,user.getVRequest());            
            statement.setString(7,user.getGender());
            statement.setString(8,user.getUsrtype());
	...
	// Unfollow a user
	public void unfollowUser(Integer follower, Integer followed) {
	...
	public List<User> getUsers(Integer start, Integer end) {
		 String query = "SELECT id,usr FROM users ORDER BY name ASC LIMIT ?,?;";
		 PreparedStatement statement = null;
	...
	public List<User> getNotFollowedUsers(Integer id, Integer start, Integer end) {
		 String query = "SELECT id,usr FROM users WHERE id NOT IN (SELECT id FROM users,follows WHERE id = followed AND follower = ?) AND id <> ? ORDER BY usr LIMIT ?,?;";
		 PreparedStatement statement = null;
		 List<User> l = new ArrayList<User>();
		 try {
			 statement = db.prepareStatement(query);
	...
	public List<User> getFollowedUsers(Integer id, Integer start, Integer end) {
		 String query = "SELECT id,usr FROM users,follows WHERE id = followed AND follower = ? ORDER BY usr LIMIT ?,?;";
		 PreparedStatement statement = null;
		 List<User> l = new ArrayList<User>();
		 try {
			 statement = db.prepareStatement(query);
	...
	public Pair<Boolean,User> checkLogin(User user) {
		...
		String query = "SELECT id,mail,usrType from users where usr=? AND pwd=?";
		PreparedStatement statement = null;
		...
		try {
			...
			if (rs.next()) {
				user.setId(rs.getInt("id"));
				user.setMail(rs.getString("mail"));
				user.setUsrtype(rs.getString("usrType"));
				output = true;
			} 
			rs.close();
			statement.close();
