...
public class RegisterController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	   ...
	   try {
		   ...
		   if (user.isComplete() && !manager.checkUser(user.getUser()) && !manager.checkMail(user.getMail())) {
			   user.setError(4,false);
			   user.setError(1,false);
			   ...
			   if (manager.checkUser(user.getUser())) {
				   user.setError(1,true);
			   }
			   if (manager.checkMail(user.getMail())) {
